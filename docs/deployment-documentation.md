...
The wrapper runs the documentation gate **before** the command, executes argv without a shell, withholds the report token from the child process, and records its real exit outcome afterward. Changing tracked source, creating nonignored source files or changing HEAD during the command forces a failed receipt requiring reconciliation. Ignored dependency/build outputs are not treated as source changes. Manual source identity is operator-attested; the actual deployment procedure must verify the artifact/source it uses. The command must deploy this exact checkout/artifact and perform its target health checks before returning success. The wrapper cannot independently inspect arbitrary external deployments or establish that a command really used the declared source. Do not wrap a build-only command and call it a deployment.
